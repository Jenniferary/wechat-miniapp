<template>
    <div class="resume-page">
      ...
      <div class="sidebar">
        <h2>👨‍🍳 工资信息</h2>
        <ul>
          <li @click="$router.push('/waiter-profile')">个人档案</li>
          ...
          <li @click="$router.push('/waiter-leaving-working')">离职申请</li>
  ...
  export default {
    ...
    methods: {
      async fetchSalaryList() {
        try {
          const res = await fetch(`/api/salaries/list?employeeId=${this.waiterId}&employeeType=waiter`);
          const json = await res.json();
          if (json.status === "success") {
            this.salaries = json.data;
          } else {
      ...
      async generateSalary() {
        try {
          const res = await fetch(
            `/api/salaries/generate?employeeId=${this.waiterId}&employeeType=waiter&branchId=1&month=${this.month}`,
            { method: "POST" }
          );
          const json = await res.json();
          if (json.status === "success") {
      ...
      async showDetail(month) {
        ...
        try {
          const res = await fetch(
            `/api/salaries/detail?employeeId=${this.waiterId}&employeeType=waiter&month=${month}`
          );
          const json = await res.json();
          if (json.status === "success") {
