<template>
  ...
  try {
    const employeeId = this.chefId;
    const employeeType = 'chef';
    ...
    alert("请求错误：" + e.message);
      ...
      async generateSalary() {
        try {
          const res = await fetch(
            `/api/salaries/generate?employeeId=${this.chefId}&employeeType=chef&branchId=1&month=${this.month}`,
            { method: "POST" }
          );
          const json = await res.json();
          if (json.status === "success") {
      ...
      async showDetail(month) {
        ...
        try {
          const res = await fetch(
            `/api/salaries/detail?employeeId=${this.chefId}&employeeType=chef&month=${month}`
          );
          const json = await res.json();
          if (json.status === "success") {
