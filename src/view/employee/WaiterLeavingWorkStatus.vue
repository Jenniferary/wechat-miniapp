<template>
  <div class="status-page">
    <div class="sidebar">
      <h2>📌 查看离职申请进度</h2>
      <ul>
        <li @click="$router.push('/waiter-dashboard')">返回主页</li>
          ...
          <tr v-for="request in leaveRequests" :key="request.id">
            ...
            <td>
              <button
                ...
                @click="confirmLeave(request.username)"
                class="btn-approve"
              >
                确认离职
              </button>
...
<script>
export default {
  name: "WaiterLeavingStatus",
  data() {
    return {
      leaveRequests: [],  // 存储当前服务员的离职申请
    };
  },
  created() {
  ...
  methods: {
    ...
    async loadLeaveRequests() {
      try {
        const waiterId = localStorage.getItem("waiterId");  // 获取当前服务员ID
        if (!waiterId) {
          alert("未登录");
        ...
        // 调用后端接口获取该服务员的离职申请
        const res = await fetch(`/api/leaving-working/waiter-by-applicant?applicantId=${waiterId}`);
    ...
    async confirmLeave() {
  const waiterId = parseInt(localStorage.getItem("waiterId"), 10);  // 将获取到的 waiterId 转为整数
  if (!waiterId) {
    alert("未登录或无效的员工ID");
    this.$router.push("/login");
    return;
  ...
  if (confirm("确认离职该员工吗？")) {
    try {
      // 发送删除请求，传递整数类型的 waiterId
      const res = await fetch(`/api/leaving-working/waiters/delete-by-id/${waiterId}`, {
        method: "DELETE",
      });
