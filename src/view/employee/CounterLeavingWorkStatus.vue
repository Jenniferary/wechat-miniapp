          ...
          <tr v-for="request in leaveRequests" :key="request.id">
            ...
            <td>
              <button
                ...
                @click="confirmLeave(request.employeeId)"
                class="btn-approve"
              >
                确认离职
              </button>
...
export default {
  ...
  methods: {
    ...
    async confirmLeave(counterId) {
      const counterIdParsed = parseInt(localStorage.getItem("counterId"), 10);  // 将获取到的 counterId 转为整数
      if (!counterIdParsed) {
        alert("未登录或无效的员工ID");
        this.$router.push("/login");
        return;
