      ...
      <table v-if="requests.length">
        ...
        <tbody>
          <tr v-for="req in requests" :key="req.requestId">
            ...
            <td>
              <button v-if="req.status === 'HR审批通过待店长审批'" @click="approve(req.requestId)" class="btn-approve">通过</button>
              <button v-if="req.status === 'HR审批通过待店长审批'" @click="reject(req.requestId)" class="btn-reject">驳回</button>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  methods: {
    ...
    async fetchPendingRequests() {
      try {
        ...
        if (!managerId) throw new Error("未登录");
         const managerRes = await fetch(`/api/branch-managers/${managerId}`);
        const managerJson = await managerRes.json();
        if (managerJson.status !== "success") throw new Error("获取店长信息失败");
        const branchId = managerJson.data.branchId;
        // 请求当前店长待审批申请
    ...
    async approve(requestId) {
      try {
        const res = await fetch(`/api/onboarding/${requestId}/status?status=店长审批通过已正式入职`, {
          method: "PUT",
        });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        alert("店长审批通过，状态已更新为已正式入职");
        this.fetchPendingRequests();
      } catch (err) {
